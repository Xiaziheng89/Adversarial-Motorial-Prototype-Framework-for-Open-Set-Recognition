...
import datetime
from core import train, train_cs, train_cs_ampf, train_cs_ampf_plus, test
from pprint import pprint
from core.utils import save_networks
from core.consistency_detection import consistency_detection
...
# resnet18:512 resnet152:2048 wide_resnet:256 trans:768
...
parser.add_argument('--loss', type=str, default='SLCPLoss', help="Softmax,GCPLoss,RPLoss,ARPLoss,AMPFLoss,SLCPLoss")
...
def main_worker(options):
    ...
    for epoch in range(options['max_epoch']):
        ...
        if options['cs']:
            if options['loss'] == 'ARPLoss':
                train_cs(net_c, net_d, net_g, criterion, criterion_bce, opt_c, opt_d, opt_g, train_loader, **options)
            if options['loss'] == 'AMPFLoss':
                train_cs_ampf(net_c, net_d, net_g, criterion, criterion_bce, opt_c, opt_d, opt_g, train_loader,
                              epoch=epoch, **options)
        if options['cs++']:
            train_cs_ampf(net_c, net_d, net_g, criterion, criterion_bce, opt_c, opt_d, opt_g, train_loader,
                          epoch=epoch, **options)
            train(net_c, criterion, opt_c, train_loader, epoch, **options)
            train_cs_ampf_plus(net_c, net_g2, criterion, opt_c, opt_g2, train_loader, epoch=epoch, **options)
...
if __name__ == '__main__':
    ...
    my_option = vars(args)
    consistency_detection(**my_option)

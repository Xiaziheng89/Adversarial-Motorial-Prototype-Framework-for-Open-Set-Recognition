...
def train_wrns(net, params, criterion, optimizer, train_loader, epoch=None, **options):
    # net.train()
    losses = AverageMeter()
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        with torch.set_grad_enabled(True):
            optimizer.zero_grad()
            x, y = net(data, params, mode=True, return_feature=True)
...
def train_cs_ampf_plus(net, net_generator_2, criterion, optimizer, optimizer_generator_2, train_loader, epoch=None,
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        x, y = net(data, True, 0 * torch.ones(data.shape[0], dtype=torch.long).cuda())
        _, loss = criterion(x, y, labels)        # cross entropy loss
...
def train_cpl(net_c, net_g, net_d, criterion, criterion_bce, criterion_l1, opt_c, opt_g, opt_d, train_loader,
              **options):
    net_c.train()
    net_g.train()
    net_d.train()
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        if options['use_gpu']:
            data, labels = data.cuda(), labels.cuda()
        valid = Variable(torch.tensor(data.shape[0], 1).fill_(1.0), requires_grad=False)
        fake = Variable(torch.tensor(data.shape[0], 1).fill_(0.0), requires_grad=False)
        data, labels = Variable(data), Variable(labels)

        # 1. update classifier(encoder):
        opt_c.zero_grad()
        feature_z, y = net_c(data, True)
        logic, loss_c = criterion(feature_z, y, labels)
        loss = loss_c + options['lambda'] * criterion_bce(net_d(feature_z), valid)
        loss.backward()
        opt_c.step()

        # 2. update generator(decoder):
        opt_g.zero_grad()
        feature_z, _ = net_c(data, True)
        fake_data = net_g(feature_z)
        loss_g = criterion_l1(fake_data, data)
        loss_g.backward()
        opt_g.step()

        # 3. update discriminator:
        opt_d.zero_grad()
        noise = Variable(torch.tensor(np.random.normal(0, 1, (data.shape[0], options['feature_dim']))))
        real_loss = criterion_bce(net_d(noise), valid)
        fake_loss = criterion_bce(net_d(feature_z.detach()), fake)
